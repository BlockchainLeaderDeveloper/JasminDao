...
export const loadAppDetails = createAsyncThunk(
  ...
  async ({ networkID, provider }: IBaseAsyncThunk, { dispatch }) => {
    ...
    try {
      ...
      console.log('originalPromiseResult1',networkID)
      console.log('originalPromiseResult2',provider)
    ...
    const busdPALContract = new ethers.Contract(addresses[networkID].BUSD_PAL_ADDRESS as string, ierc20Abi, provider);
    ...
    let busdTreasuyBalance = await busdContract.balanceOf(addresses[networkID].TREASURY_ADDRESS);
    let busdPALTreasuyBalance = await busdPALContract.balanceOf(addresses[networkID].TREASURY_ADDRESS);
    busdTreasuyBalance = parseFloat(ethers.utils.formatUnits(busdTreasuyBalance, "ether"));
    busdPALTreasuyBalance = parseFloat(ethers.utils.formatUnits(busdPALTreasuyBalance, "ether"));

    console.log('debug bal', hecBalance)
   ...
   //  currentIndex = currentIndex / 1000000000;
    ...
    console.log('debug staking reward, rebase, five, apy,currentIndex', stakingReward, stakingRebase, fiveDayRate, stakingAPY,currentIndex,currentBlock)
    return {
...
const loadMarketPrice = createAsyncThunk("app/loadMarketPrice", async ({ networkID, provider }: IBaseAsyncThunk) => {
  ...
  try {
    ...
    console.log('marketPric2',provider)
  } catch (e) {
